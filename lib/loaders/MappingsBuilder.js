...
module.exports = class MappingsBuilder {
    ...
    reload (){
        ...
        if( this.options.from ){
            ...
            this.WhenMappingNotFound.response.record = this.options.recordPath;
        }else{
    ...
    updateMappingOptions(req, resp){
        ...
        if(resp.from){
            if(resp.record === true) resp.recordPath = ""; //relative to stubs path
            /* else if(resp.record) {
                if( fileutil.isExist(resp.record) ) resp.recordPath = resp.record; //absolute path
                else{
