...
exports.buildMappings = function(config){
    if( config.redirectTo ){
        proxyMapping.response.proxy = config.redirectTo;
        allMappings = [ proxyMapping ];
    }else{
        var mappings;
        allMappings = [];
            ...
            try{
                ...
                if(!mappings || mappings.length === 0){
                    ...
                    for(var i=0;i<mappings.length;i++){
                        ...
                        if(mappings[i].response && mappings[i].response.parser && typeof mappings[i].response.parser === "string"){
                            mappings[i].response.parser = {
                                type : mappings[i].response.parser
                            };
                        }
                        //response
...
exports.setDefaultMappings = function(mappings) {
