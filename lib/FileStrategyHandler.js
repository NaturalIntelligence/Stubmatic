...
module.exports = class FileStrategyHandler{
	...
	resolveName(fileEntry, matches) {
		...
		if (typeof fileEntry === 'object') {
			const resolvedName = resolveParsedRequestExpression(fileEntry.name, matches)
			if(fileutil.isExist(resolvedName)){
				fileEntry.name = resolvedName;
			}else{
				fileEntry.name = path.join(stubsPath, resolvedName);
			}
		} else {
			const resolvedName = resolveParsedRequestExpression(fileEntry, matches)
			if(fileutil.isExist(resolvedName)){
				fileEntry = resolvedName;
			}else{
				fileEntry = path.join(stubsPath, resolvedName);
			}
		}
		return fileEntry;
	}
