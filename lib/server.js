...
function redirect(request, response, requestProcessor, baseUrl, options){
	...
	setTimeout(function(){
		...
		proxyModule.redirect(reqOptions, response, function( responseRecieved){
			...
			if(options.recordPath && responseRecieved.status > 399){
				
				//record input request as mapping
				const reqResponseMapping = buildMappingFileContent(request, responseRecieved);
				...
				const fileDetail = util.fileNameFromUrl(request.originalUrl, 30);
				if(!fileDetail.ext){
				...
				let recordingPath = options.recordPath;
